...
// app opens the "C4D Portal" camera). A named file mapping + named mutex
// are used because the two sides run in different processes but the same
// user session — see docs/protocol.md "Frame delivery to the virtual camera".
//
...
#pragma pack(pop)
  ...
  // Reader side: called from C4DPortalMediaSource.dll when serving a
  // sample. Returns false only if no frame has EVER been written —
  // otherwise it keeps returning the most recent frame (repeating it) even
  // if it isn't new since the last call. The consuming stream's declared
  // framerate (e.g. 30fps) is usually higher than however often the
  // phone/desktop actually pushes updates, so gating on "new frame only"
  // made most RequestSample calls fall back to the gray placeholder —
  // visible as flicker. Repeating the last frame is what real
  // cameras/virtual cams do between source updates.
